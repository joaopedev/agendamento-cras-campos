...
const SelecionarDia: React.FC = () => {
  ...
  const { getSchedulling, payload, registerSchedulling, getUser } =
  ...
  const onSubmit: SubmitHandler<RegisterSchedulling> = async (
    data: RegisterSchedulling
  ) => {
    try {
    ...
    return horarios.map((horario) => {
      if (selectedDate) {
        ...
        const horariosAgendados = schedullingData
          .filter(
            (agendamentos) =>
              format(agendamentos.data_hora, "yyyy-MM-dd") ===
                      ...
                      <Flex flexDir="column" alignItems="center">
                        ...
                        <FormControl isInvalid={!!errors.name}>
                          ...
                          <Input
                            id="name"
                            {...register("name")}
                            defaultValue="Agendamento do jassa"
                          />
                          {errors.name && (
                        ...
                        <FormControl isInvalid={!!errors.duracao_estimada}>
                          <FormLabel htmlFor="duracao_estimada">
                            Duração Estimada
                          </FormLabel>
                          <Input
                            id="duracao_estimada"
                            {...register("duracao_estimada")}
                            defaultValue="2024-06-26T14:00:00"
                          />
                          {errors.duracao_estimada && (
                            <Text color="red.500">
                              {errors.duracao_estimada.message}
                            </Text>
                          )}
                        </FormControl>
