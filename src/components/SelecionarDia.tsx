...
const SelecionarDia: React.FC = () => {
	...
	const hoje = new Date();
	const maxDate = payload?.tipo_usuario === 1 ? addDays(new Date(), 7) : addDays(new Date(), 600);
	...
	const bgColor =
		payload?.tipo_usuario !== 1 ? 'hsla(207, 74%, 42%, 1)' : 'hsla(145, 100%, 29%, 1)';
	...
	const onSubmit: SubmitHandler<RegisterSchedullingModel> = async (
		...
		try {
			...
			toast({
				title: 'Este horário não está mais disponível, por favor escolha outro horário.',
				status: 'error',
					...
					{selectedDate && (
						...
						>
							...
							{isBeforeNineAm ? (
								...
								<Box p={4} borderWidth="1px" borderRadius="md" bg="gray.100" mt={4}>
									<Text fontWeight={'bold'} fontSize={['xl', 'xl', '2xl', '3xl']}>
