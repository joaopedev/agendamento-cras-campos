...
const SelecionarDiaAdm: React.FC = () => {
	const crasValues = Object.values(Cras).filter(value => typeof value === 'string') as string[];
	const [selectedAll, setSelectedAll] = useState(false);
	const [selectedCras, setSelectedCras] = useState<number[]>([]);
	const [selectedItems, setSelectedItems] = useState<Record<string, boolean>>(
		crasValues.reduce((acc, cras) => ({ ...acc, [cras]: false }), {})
	);
	const [showConfirmar, setShowConfirmar] = useState(false);
	const [showCancelar, setShowCancelar] = useState(false);
	const [showCheckox, setShowCheckox] = useState(true);
	...
	const [periodoSelecionado, setPeriodoSelecionado] = useState<'manha' | 'tarde' | 'dia_inteiro'>();
	const [periodoReservado, setPeriodoReservado] = useState<
		'manha' | 'tarde' | 'dia_inteiro' | null
	>(null);
	const showBlueButtons = differenceInDays(selectedDate, new Date()) >= 0;
	...
	// Função para lidar com a seleção de todos os checkboxes
	const handleSelectAll = () => {
		const newSelectedAll = !selectedAll;
		setSelectedAll(newSelectedAll);

		const updatedItems = Object.fromEntries(crasValues.map(cras => [cras, newSelectedAll]));
		setSelectedItems(updatedItems as Record<string, boolean>);

		const allIds = newSelectedAll ? crasValues.map(cras => Cras[cras as keyof typeof Cras]) : [];
		setSelectedCras(allIds);
	};
	...
	// Função para lidar com a seleção individual de checkboxes
	const handleCheckboxChange = (cras: string) => {
		const crasId = Cras[cras as keyof typeof Cras];
		const isSelected = selectedItems[cras];
		...
		const updatedSelectedCras = isSelected
			? selectedCras.filter(id => id !== crasId)
			: [...selectedCras, crasId];
		setSelectedCras(updatedSelectedCras);

		setSelectedAll(updatedSelectedCras.length === crasValues.length);
	};

	const handleClose = () => {
		setSelectedAll(false);
		setSelectedItems(crasValues.reduce((acc, cras) => ({ ...acc, [cras]: false }), {}));
		setMotivo('');
	...
	const confirmarBloqueio = async () => {
		if (!selectedCras.length || !selectedDate || !periodoSelecionado || !payload?.id) return;

		const tipoBloqueio = mapPeriodoToTipoBloqueio(periodoSelecionado);

		// Cria o array de bloqueios para enviar
		const bloqueiosData = selectedCras.map(crasId => ({
		...
		try {
			// Passa o array para `registerBlock` diretamente
			await registerBlock(bloqueiosData);
			toast({
				title: 'Reservas realizadas com sucesso',
				duration: 5000,
				isClosable: true,
				position: 'top-right',
				status: 'success',
			...
			toast({
				title: 'Erro ao realizar reservas',
					...
					<Flex gap={2} flexDirection={'column'}>
						...
						<Flex flexDir={'column'} justifyContent={'center'}>
							...
							<Text fontSize={['xl', '2xl', '3xl', '4xl']}>
								<strong>
									{closestDate ? format(closestDate, 'dd/MM/yy') : 'Nenhuma reserva futura'}
								</strong>
							</Text>
						</Flex>
						<Modal
						...
						>
							...
							{showCheckox && (
								<ModalContent minW={['90%', '27em', '30em', '48em']} textAlign={'center'}>
									<ModalHeader mt={5}>SELECIONE OS CRAS QUE DESEJA RESERVAR:</ModalHeader>
									...
									<ModalBody display={'flex'} flexDir={'column'}>
										...
										{crasValues.map((cras, index) => (
											<Checkbox
												key={index}
												isChecked={selectedItems[cras]}
												onChange={() => {
													handleCheckboxChange(cras);
													setShowAvancar(true);
												}}
											>
												{cras}
